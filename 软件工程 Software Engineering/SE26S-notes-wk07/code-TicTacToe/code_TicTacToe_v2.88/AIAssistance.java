...
public class AIAssistance {
  ...
  protected Move obviousMove () {
    ...
    if (moves.size() > 0) return anyMoveFrom( moves);
  ...
  protected Move smartMove () {
    ...
    if (moves.size() == 0)   // smart starting :-)
      return generator.nextBoolean() ? 
        anyMoveFrom( Move.CORNERS) :
        (generator.nextBoolean() ? anyMoveFrom( Move.SIDES) : Move.CENTER);
  
    if (moves.size() == 3 && moves.get(0).isCatercorner( moves.get(2))) 
      return anyMoveFrom( Move.SIDES);  // The unique special case!
    ...
    return anyMoveFrom( maxCrossMoves());
  } 
  
  private static Random generator = new Random();
  private Move anyMoveFrom (ArrayList<Move> moves) {
    return moves.get( generator.nextInt( moves.size()));
  }
  private Move anyMoveFrom (Move[] moves) {
    ...
    for (int r = 1; r <= Board.SIZE; r++) {
      for (int c = 1; c <= Board.SIZE; c++) {
        ...
        if (board.isValid( move)) {
          ...
          ArrayList<Move> threatMoves = 
            nextDoubleThreatsMoves( opposite);
